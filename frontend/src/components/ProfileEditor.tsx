...
export function ProfileEditor({
  ...
  const value = (key: keyof ProfileDraft) => ({
    ...
    onChange: (event: React.ChangeEvent<HTMLInputElement>) => setDraft((current) => current && ({
      ...current,
      [key]: ['profile_number', 'pacs_server_port', 'storage_port'].includes(key)
  ...
  return <Sheet open={open} onOpenChange={onOpenChange} title={launchAfterSave ? '配置并启动 Profile' : '配置 Profile'} description="每个 Profile 使用独立的 DICOM 接收端口；工作台内部连接由系统自动管理。" footer={<Button type="submit" form="profile-editor" variant="primary" disabled={busy}><Save size={17} />{busy ? '保存中…' : launchAfterSave ? '保存并启动' : '保存配置'}</Button>} size="normal">
