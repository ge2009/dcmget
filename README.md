...
应用仍支持多个持久 Profile（`i1`、`i2`……）。Windows 安装版从 3.2.0 起提供固定使用 `8786` 的独立管理中心，用一个页面查看所有 Profile 的运行状态、配置冲突和入口链接。每个 Profile 仍是独立后台进程，配置、Web 端口、接收 AE/端口、任务恢复点和日志互相隔离。不同 Profile 必须使用不同的 Web 端口和接收端口，通常也应配置独立接收 AE，并在 PACS 中分别建立 Move Destination 映射。收到的文件按可配置的 DICOM 元数据目录归档，下载结束后可自动生成包含原始 DICOM、`DICOMDIR`、中文 OHIF 和本地只读 HTTP 启动器的 PDI 便携目录。
...
- `DcmGet-3.2.0-Setup-x64.exe`：默认推荐的一键安装器，内置 x64 Python 运行时、FastAPI/Uvicorn、离线 Web 控制台、精简的 x64 DCMTK 3.7.0 运行集、离线中文 OHIF、PDI 本地只读 HTTP 启动器、Microsoft Visual C++ x64 Runtime 和 `kayisoft-dcmget` Windows 服务。服务随系统自动启动，先托管独立管理中心，再托管现有 Profile；安装器创建 `DcmGet Web TCP` 与 `DcmGet Receiver TCP` 两条仅限域/专用网络的程序级入站规则。
- `DcmGet-3.2.0-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动会把逐文件 SHA-256 校验通过的精简 x64 DCMTK 运行集发布到 `%LOCALAPPDATA%\DcmGet\runtime\dcmtk\<版本与清单哈希>\`，以后启动和软件升级会复用内容相同的稳定用户级路径。两个进程同时首次启动时也只会原子发布一份运行集。PDI 同样使用原始 DICOM 和离线 OHIF。便携版不注册 Windows 服务，也不自动常驻管理中心。
- `DcmGet-3.2.0-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 Web 控制台、精简 DCMTK 运行集、离线 OHIF 和 PDI 启动器，但不自动注册 Windows 服务或常驻管理中心。
...
安装完成后，`kayisoft-dcmget` 以 LocalSystem 自动服务运行，但固定使用安装用户的 `%APPDATA%`、`%LOCALAPPDATA%` 和 `%USERPROFILE%`，因此会继续读取原有 Profile、恢复点和配置。桌面和开始菜单的“DcmGet”主入口会打开 `http://127.0.0.1:8786/?page=operations`；开始菜单仍提供“DcmGet 启动全部”和“DcmGet 停止全部”。内置服务权限允许普通本机用户查询、启动和停止服务，不需要应用密码或再次提升权限。注册、升级和卸载 Windows 服务本身仍属于系统级安装操作，首次运行安装器时需要管理员/UAC。

同一局域网的管理电脑可打开 `http://<DcmGet Windows 主机 IP>:8786/?page=operations`。管理中心会沿用浏览器当前访问的主机地址，将“任务”“设置”和“管理”按钮链接到各 Profile 的独立 Web 端口，不会误跳到访问者自己的 `127.0.0.1`。为避免与管理中心冲突，Profile 的 SCP 和 Web 端口均不能使用 `8786`。

服务启动时会接管当时已经配置完成的全部 Profile，并在子进程异常退出后重启对应 Profile。新建 Profile 会先留在停止状态，完成 PACS、AE、SCP/Web 端口和目标目录配置并点击“保存并启动”后，`kayisoft-dcmget` 会在不重启服务的情况下自动接管该进程；后续崩溃会自动拉起，删除配置时也会停止并移出监管。若直接点击 `.url` 后浏览器提示无法连接，请先点击“DcmGet 启动全部”。
...
python scripts/build_windows.py --version 3.2.0
```
...
- Windows 安装版管理中心固定监听 `0.0.0.0:8786`；首个 Profile 默认监听 `0.0.0.0:8787`，后续 Profile 使用各自的 `web_port`。本机优先从 `http://127.0.0.1:8786/?page=operations` 进入，局域网管理端使用 Windows 主机 IP 访问同一端口。
...
- DCMTK 启动失败：下载需要同一套 `storescp` 和 `movescu`，PDI 还需要 `dcmmkdir`、`dcmdump`。Windows 3.2.0 成品已包含这四个程序及所需 DLL、字符集和许可证数据，不包含 `dcmj2pnm`、`dcmdjpeg` 等未使用工具；源码部署或自定义路径请选择同时包含这四个程序的 DCMTK 3.7.0 `bin` 目录。
