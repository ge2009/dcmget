...
- `DcmGet-2.6.5-Setup-x64.exe`：默认推荐的一键安装器，内置 Python 运行时、PyQt5、DCMTK 3.7.0、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并创建仅允许实际 `storescp.exe` 在域/专用网络建立入站连接的程序级防火墙规则，兼容设置中的自定义接收端口。
- `DcmGet-2.6.5-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动需要等待程序解压运行环境，PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.6.5-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 2.6.5
```
...
8. 如需交付 U 盘，在设置中启用 PDI、填写机构名称并保持“在目录内附带离线阅片器（推荐）”勾选；批次结束后点击“打开影像”，或用“打开导出目录”将整个目录复制到 U 盘。重启程序后也可点击“打开已有 PDI 目录”直接选择根目录阅片，无需寻找 JSON 文件。PDI 失败可单独重试，无需重新下载。
