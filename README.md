...
新建 Profile 默认保持停止。管理中心启动时不会根据旧服务留下的运行意图自动启动 Profile；每个 Profile 都保持停止，直到用户在当前会话中明确启动。用户明确停止后则保持停止，删除 Profile 前也必须先停止。切换当前 Profile 只改变右侧页面内容，不会停止其他正在执行的下载任务。
