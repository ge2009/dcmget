...
- `DcmGet-2.9.0-Setup-x64.exe`：默认推荐的一键安装器，内置 Python 运行时、PyQt5、DCMTK 3.7.0、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并为实际监听的 `storescp.exe` 创建仅限域/专用网络的程序级入站防火墙规则；规则不固定端口，可兼容每个实例自己的接收端口。
- `DcmGet-2.9.0-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动会把逐文件 SHA-256 校验通过的 DCMTK 发布到 `%LOCALAPPDATA%\DcmGet\runtime\dcmtk\<版本与清单哈希>\`，以后启动和软件升级会复用内容相同的稳定用户级路径。两个进程同时首次启动时也只会原子发布一份完整运行时。PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.9.0-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 2.9.0
```
...
| `max_concurrent_moves` | 仅为兼容 2.8.x 配置保留，2.9.0 不再生效；每个实例始终顺序执行 C-MOVE |
...
旧版配置会自动迁移到 v6；原 PACS、AE、目录、PDI 机构名称和输出位置会保留。`max_concurrent_moves` 字段仅为读取旧配置兼容而保留，2.9.0 不显示该设置，也不会据此并发 C-MOVE。首次创建 `i1` 时会从现有配置初始化；后续新实例以首个实例配置为模板，再由用户分别修改接收 AE、端口、目标目录等字段。v5 的 OHIF 迁移语义保持不变：旧 JPEG 预览和 Weasis 选项均关闭时继续关闭，任一开启时启用 OHIF，迁移后不再保存旧字段。DCMTK 的查找顺序是：实例配置目录、`.runtime/dcmtk` 部署目录、旧版 `dcmtk/bin`、系统 `PATH`。
