...
- `DcmGet-2.6.2-Setup-x64.exe`：默认推荐的一键安装器，内置 Python 运行时、PyQt5、DCMTK 3.7.0、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并创建仅允许实际 `storescp.exe` 在域/专用网络使用默认端口 6666 的入站防火墙规则。
- `DcmGet-2.6.2-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动需要等待程序解压运行环境，PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.6.2-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 2.6.2
```
...
8. 如需交付 U 盘，在设置中启用 PDI、填写机构名称并保持“生成后可直接阅片（推荐）”勾选；批次结束后点击“立即阅片”，或用“打开文件夹”将整个目录复制到 U 盘。PDI 失败可单独重试，无需重新下载。
...
- 用户只需打开 PDI 根目录或点击 DcmGet 中的“立即阅片”，不需要选择 JSON 或其他索引文件。程序会通过内部只读接口加载预先校验的阅片数据，避免每次启动重新扫描 U 盘中的全部 DICOM。
...
- 阅片过程不联网：阅片资源、内部索引和 DICOM 影像均从当前 PDI 目录读取，不使用外部 CDN、在线 DICOMweb 服务或远程账号。
- DcmGet 会等本地目录服务通过就绪检查后再打开浏览器；服务连续空闲约 4 小时才自动退出。再次点击“立即阅片”可重新打开已经运行的页面。
