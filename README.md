...
DcmGet 是一个跨平台 DICOM C-MOVE 多任务工作台。单个应用窗口可同时管理多个下载任务，全局共享一个基于 `pynetdicom` 的 C-STORE 接收器，并行执行多个 `movescu`；默认并发 2 个检查号，可在 1-8 之间设置。接收器按检查号和已绑定的 Study Instance UID 将影像写入独立暂存目录，避免并发任务串片。收到的文件按可配置的 DICOM 元数据目录归档，下载结束后可自动排队生成包含原始 DICOM、`DICOMDIR`、中文 OHIF 和本地只读 HTTP 启动器的 PDI 便携目录。界面和命令行共用同一套配置、持久化、进程管理、下载与导出核心。
...
- `DcmGet-2.8.0-Setup-x64.exe`：默认推荐的一键安装器，内置 Python 运行时、PyQt5、pynetdicom、DCMTK 3.7.0、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并创建仅允许实际 `DcmGet.exe` 在域/专用网络建立入站连接的程序级防火墙规则，兼容设置中的自定义接收端口。
- `DcmGet-2.8.0-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动需要等待程序解压运行环境，PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.8.0-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 2.8.0
```
...
部署脚本会检查 Python 版本、重建干净的 `.venv`、安装 PyQt5 和 pynetdicom、下载 DCMTK 3.7.0、准备离线 OHIF 和 PDI 本地启动器、检查 VC++ Runtime，并在管理员模式下为该虚拟环境的 Python 接收进程创建入站防火墙规则。重建虚拟环境不会改动配置、注册码、试用记录或下载结果。
...
5. 每个任务可独立暂停、继续、停止、重试失败项、打开目标目录和打开 PDI。暂停任务会在当前检查号安全结束后停止领取新检查号，不影响其他任务；继续后重新加入并发调度。停止或退出不会删除已收到的文件。
6. 每个任务不超过 200 个检查号时显示逐项文件数、速度和耗时；超过 200 个时只显示聚合进度和状态统计，避免 40,000 条任务拖慢界面。所有归档文件统一以 `.dcm` 结尾。
7. 日志可以在“本任务/全部”之间切换；每个任务拥有独立日志，共享接收器写入单独的接收器日志。
...
- 全局共享一个跨平台 pynetdicom C-STORE SCP，同一个接收 AE 和端口可同时处理多个 association。调度器默认允许 2 个、最多 8 个 C-MOVE 并行运行；每次公平地从不同可运行任务领取一个检查号，空闲槽会立即接纳新任务。存在可运行任务时接收器自动启动，全部下载任务暂停或结束后自动停止。
...
- 每个任务保存创建时的完整配置快照。PACS、DCMTK、接收 AE 和端口属于应用级设置，存在未完成任务时会锁定；目标目录、匿名化和 PDI 默认值的修改只影响之后创建的任务。不同未完成任务不能包含相同检查号，已结束的历史任务不阻止重新下载。
...
主界面顶部的“诊断日志”可打开该固定目录；发生闪退时请同时提供上述两个文件。多任务下载会为每个任务写入 `task-<任务编号>.log`，共享接收器写入独立的 `receiver.log`；匿名模式的日志写入应用私有状态目录。macOS 源码启动还会在创建界面前自动清除 iCloud 可能附加到平台插件的 hidden 标志，避免 Qt 找不到 Cocoa 插件后直接退出。

共享接收器在应用私有状态目录为每个活动检查号创建独立暂存区。程序确认 pynetdicom 已监听后，再并行执行带连接与 DIMSE 超时的 `movescu --no-port`。每个 C-STORE 对象必须精确匹配活动检查号，或匹配此前由该检查号绑定的 Study Instance UID；只有明确把并发数设为 `1` 时，才允许将缺失检查号但带 Study UID 的旧 PACS 对象绑定到唯一活动路由。默认并发模式即使瞬时只剩一个任务，也绝不按时间猜测归属。每条 C-MOVE 完成后读取 DICOM 元数据，按任务配置快照中的目录模板归档并补充 `.dcm` 后缀；关键元数据缺失时使用安全占位值，无法归属的文件进入隔离目录，匿名失败文件会保留并写入日志。
