...
- `DcmGet-2.11.0-Setup-x64.exe`：默认推荐的一键安装器，内置 x64 Python 运行时、PyQt5、精简的 x64 DCMTK 3.7.0 运行集、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并为实际监听的 `storescp.exe` 创建仅限域/专用网络的程序级入站防火墙规则；规则不固定端口，可兼容每个实例自己的接收端口。
- `DcmGet-2.11.0-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动会把逐文件 SHA-256 校验通过的精简 x64 DCMTK 运行集发布到 `%LOCALAPPDATA%\DcmGet\runtime\dcmtk\<版本与清单哈希>\`，以后启动和软件升级会复用内容相同的稳定用户级路径。两个进程同时首次启动时也只会原子发布一份运行集。PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.11.0-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的精简 DCMTK 运行集、离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 2.11.0
```
...
部署脚本会检查 AMD64/x64 Python 3.10+、拒绝 32 位和原生 ARM64 Python、重建干净的 `.venv`、安装运行依赖、下载 x64 DCMTK 3.7.0、准备离线 OHIF 和 PDI 本地启动器、检查 VC++ Runtime，并在管理员模式下为实际监听的 `storescp.exe` 创建仅限域/专用网络的程序级入站防火墙规则；该规则不固定端口，可覆盖不同实例配置的接收端口。Windows 11 ARM64 用户应安装 x64 Python 并通过系统兼容层运行。重建虚拟环境不会改动配置、注册码、试用记录或下载结果。
...
2. 选择或拖入 TXT、CSV 或 XLSX，也可以直接粘贴多行检查号。CSV/XLSX 可选择表头列；空行会忽略，重复项会按首次出现顺序去重，公式单元格会被拒绝。选择目标目录、目录模板和 PDI 快捷选项后开始任务。
3. 开始前会检查配置、DCMTK、目标目录、磁盘保留空间和当前实例的接收端口。`storescp` 就绪后，程序按检查号顺序执行 `movescu`。
4. 任务主页显示总进度、当前检查号、文件数、下载速度、状态统计、PDI 状态和操作按钮。可暂停、继续、停止、重试失败项、打开目标目录、打开 PDI 和查看默认脱敏的验收报告；停止或退出不会删除已收到的文件。
...
- 实例槽以 `i1`、`i2` 等编号持久保存。每个槽分别拥有配置文件、任务恢复点、Qt 窗口偏好（QSettings）和日志目录；关闭程序不会删除槽位内容。也可以用 `--profile N` 明确打开指定槽；若该槽已在运行，新启动进程会唤醒并置前原窗口，不会再创建重复任务窗口。
- 主界面的“实例快捷方式”可为当前槽生成固定使用 `--profile N` 的桌面快捷方式。默认名称取当前配置中的接收端口和接收 AE，例如 `dcmget-6666-DCMGET`，保存前可以修改；以后点击该快捷方式会直接打开对应实例，实例已运行时同样只唤醒原窗口。安装器创建的通用 DcmGet 快捷方式仍按正常规则自动选择可用实例槽。
...
桌面 GUI 使用持久实例槽：未指定 `--profile` 时自动取得空闲或待恢复槽；指定 `--profile N` 时只使用对应槽，若该槽已被另一个进程占用则唤醒并置前现有窗口后退出新进程。GUI 的 `--config PATH` 只作为首次创建该 profile 时的配置初始化模板，不会把 profile 的长期身份改到模板所在目录；旧版本曾在自定义配置目录创建的 profile 会在首次打开时自动迁移到规范实例目录，并保留原文件。命令行继续作为单任务前台客户端运行，并保留显式 `--task-state` 恢复参数；`--task-id` 仅用于尚未迁移的 2.8 旧任务目录。默认旧任务目录迁移到 2.9 实例槽后，命令行会拒绝再次打开它，避免与 GUI 重复执行同一批任务。所有入口共享整机 30 次试用计数和注册码，不再要求日期口令。可用 `--license PATH` 指定注册码文件；`--accept-download-failures` 可接受当前下载结果并继续 PDI，`--discard-checkpoint` 可明确放弃所选恢复点。

`--health-check` 输出运行时、DCMTK、目录、磁盘、端口、进程和 PACS TCP 可达性检查；这是网络可达性检查，不是 DICOM C-ECHO。`--support-bundle` 生成默认脱敏的诊断 ZIP。Profile 备份包含配置和可选显示名，不包含注册码、试用计数、匿名密钥、下载结果或任务恢复点；恢复前会自动创建现有配置与显示名快照，旧版 v1 备份仍可兼容恢复。桌面界面的“运维工具 → 管理 Profile”可复制、重命名、删除安全空闲实例并创建固定 `--profile N` 快捷方式；复制时只自动推荐未占用端口，接收 AE 和 PACS 映射必须由用户确认。CLI 和桌面界面的 PDI 验证都可直接选择单卷目录，或选择含 `VOLUME_SET.json` 的分卷集根目录并严格按清单逐卷验证。
...
首次部署会从 `config.example.json` 创建 `config.json`。GUI 传入的 `--config PATH` 仅用于初始化尚未创建的 profile，之后始终读写该 profile 的规范配置目录；旧版自定义目录中的 profile 会自动迁移且不覆盖原文件。主要字段：
...
旧版配置会自动迁移到 v7，且不覆盖现有设置；原 PACS、AE、目录、PDI 机构名称和输出位置会保留。v7 仅为新增的 PDI 分卷、磁盘保护、重试和熔断字段补入安全默认值。`max_concurrent_moves` 字段仅为读取旧配置兼容而保留，自 2.9.0 起不显示该设置，也不会据此并发 C-MOVE。首次创建 `i1` 时会从现有配置初始化；后续新实例以首个实例配置为模板，再由用户分别修改接收 AE、端口、目标目录等字段。v5 的 OHIF 迁移语义保持不变：旧 JPEG 预览和 Weasis 选项均关闭时继续关闭，任一开启时启用 OHIF，迁移后不再保存旧字段。DCMTK 的查找顺序是：实例配置目录、`.runtime/dcmtk` 部署目录、旧版 `dcmtk/bin`、系统 `PATH`。
...
主界面顶部的“诊断日志”可打开该固定目录；发生闪退时请提供对应时间和进程号的诊断、崩溃文件。非匿名任务的下载与接收日志写入影像目标目录下的 `_DcmGetLogs`，便于与结果一同交付；匿名任务日志始终位于实例私有状态目录 `iN/logs`，不会泄露到结果目录。目标日志目录不可写时也会回退到该实例私有目录。macOS 源码启动还会在创建界面前自动清除 iCloud 可能附加到平台插件的 hidden 标志，避免 Qt 找不到 Cocoa 插件后直接退出。
...
- DCMTK 启动失败：下载需要同一套 `storescp` 和 `movescu`，PDI 还需要 `dcmmkdir`、`dcmdump`。Windows 2.11.0 成品已包含这四个程序及所需 DLL、字符集和许可证数据，不包含 `dcmj2pnm`、`dcmdjpeg` 等未使用工具；源码部署或自定义路径请选择同时包含这四个程序的 DCMTK 3.7.0 `bin` 目录。
