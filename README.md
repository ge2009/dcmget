...
8. 如需交付 U 盘，在设置中启用 PDI、填写机构名称并保持“在目录内附带离线阅片器（推荐）”勾选；批次结束后点击“打开影像”，或用“打开导出目录”将整个目录复制到 U 盘。PDI 失败可单独重试，无需重新下载。
...
- 用户只需打开 PDI 根目录或点击 DcmGet 中的“打开影像”，不需要选择 JSON、`DICOMDIR` 或逐个影像文件。程序会通过带会话保护的内部只读目录接口加载预先校验的阅片数据，避免每次启动重新扫描 U 盘中的全部 DICOM。
