...
- `DcmGet-2.6.1-Setup-x64.exe`：默认推荐的一键安装器，内置 Python 运行时、PyQt5、DCMTK 3.7.0、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并创建 `storescp` 默认端口 6666 的入站防火墙规则。
- `DcmGet-2.6.1-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动需要等待程序解压运行环境，PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.6.1-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 OHIF 和 PDI 启动器。
...
5. 点击“开始下载”。200 个及以内会显示每个检查号的文件数、实际接收速度和耗时；超过 200 个时自动隐藏逐项列表，只显示总进度和状态汇总。所有归档文件统一以 `.dcm` 结尾；普通模式日志位于 `保存目录/logs/`，匿名模式日志改存应用私有状态目录。
6. 点击“暂停”后，按钮会先显示“取消暂停”，程序安全完成并归档当前检查号后才显示“继续下载”；`storescp` 在暂停期间保持监听。
7. 部分失败时可点击“重试失败项”。停止或退出不会删除已收到的文件。
8. 如需交付 U 盘，在设置中启用 PDI、填写机构名称并保持“包含中文 OHIF 网页阅片器”勾选；批次结束后可打开生成目录，或在 PDI 失败时单独重试而无需重新下载。

任务每处理完一个检查号都会提交到应用私有目录中的 SQLite 恢复点。重启或意外退出后，程序会询问是否继续上次任务；已完成、无数据和已失败的检查号不会重复请求，退出时正在处理的检查号会重新请求一次。恢复默认使用任务启动时的配置和保存目录，也可在继续前修正目录或设置并写回恢复点；PDI 阶段可以单独恢复，已发布且校验通过的目录会直接复用，遗留的 DCMTK 子进程会先经过身份校验再清理。恢复同一个免费试用任务不会重复扣次数。CLI 同样会自动检测恢复点，可用 `--discard-checkpoint` 明确放弃。
...
├── DCMGET_STUDIES.json   # OHIF 使用的本地 DICOM JSON 清单
├── VIEWER/OHIF/          # 离线中文 OHIF 静态资源
└── OPEN_VIEWER.*         # 当前平台的本地阅片启动器
```

- 普通下载结果仍以 `.dcm` 结尾；PDI 内部复制件使用符合 DICOM File ID 约束的无扩展名短路径，源文件不会被修改。
- OHIF 直接读取 PDI 内的原始 DICOM，不生成 JPG 或 PNG 预览副本，也不携带 Weasis。阅片界面默认使用中文，支持 OHIF 提供的序列、窗宽窗位、测量和多帧查看能力。
- 现代浏览器不允许 OHIF 通过 `file://` 安全读取整批 DICOM，因此请运行 `OPEN_VIEWER.exe`（Windows 安装版导出的首选入口，也可用 `OPEN_VIEWER.bat`）、`OPEN_VIEWER.command`（macOS）或 `OPEN_VIEWER.sh`（Linux）。启动器只绑定 `127.0.0.1` 随机端口，只读提供当前 PDI 目录，然后打开本机浏览器。macOS/Linux 启动脚本依赖目标电脑已安装 Python 3；Windows 成品导出的 `OPEN_VIEWER.exe` 不需要另装 Python。
- 阅片过程不联网：OHIF 资源、`DCMGET_STUDIES.json` 和 DICOM 影像均从当前 PDI 目录读取，不使用外部 CDN、在线 DICOMweb 服务或远程账号。
