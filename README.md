...
- `DcmGet-3.0.1-Setup-x64.exe`：默认推荐的一键安装器，内置 x64 Python 运行时、FastAPI/Uvicorn、离线 Web 控制台、精简的 x64 DCMTK 3.7.0 运行集、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime。安装器创建 `DcmGet Web TCP` 与 `DcmGet Receiver TCP` 两条仅限域/专用网络的程序级入站规则，分别允许主程序和 `storescp.exe` 使用各 Profile 配置的端口。
- `DcmGet-3.0.1-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动会把逐文件 SHA-256 校验通过的精简 x64 DCMTK 运行集发布到 `%LOCALAPPDATA%\DcmGet\runtime\dcmtk\<版本与清单哈希>\`，以后启动和软件升级会复用内容相同的稳定用户级路径。两个进程同时首次启动时也只会原子发布一份运行集。PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-3.0.1-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 Web 控制台、精简 DCMTK 运行集、离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 3.0.1
```
...
- 按当前产品决策，3.0.0 使用裸 HTTP，不生成证书，也不启用 HTTPS。登录口令、患者相关数据和操作内容在网络中**未加密传输**，只能部署在受信任的医院内网或隔离 VLAN；禁止把端口映射到公网、访客 Wi-Fi 或不受信任网络。
- 首次启动按页面提示设置管理口令。后续访问需要登录；会话、CSRF 防护、同源检查和登录限速仍然启用，但这些机制不能替代传输加密。
...
1. 启动 DcmGet。程序在后台启动 Web 服务并用系统浏览器打开当前 Profile 地址；首次使用先设置管理口令，远端电脑访问同一地址后登录。页面顶部始终提示当前为“仅限可信内网、HTTP 未加密”。
...
5. 任务主页显示总进度、当前检查号、文件数、下载速度、状态统计、PDI 状态和操作按钮。可暂停、继续、停止、重试失败项、打开目标目录、打开 PDI 和查看默认脱敏的验收报告；关闭浏览器或退出登录不会停止后台任务，也不会删除已收到文件。
...
| `web_session_timeout_minutes` | 登录会话空闲超时时间 |
...
- DCMTK 启动失败：下载需要同一套 `storescp` 和 `movescu`，PDI 还需要 `dcmmkdir`、`dcmdump`。Windows 3.0.1 成品已包含这四个程序及所需 DLL、字符集和许可证数据，不包含 `dcmj2pnm`、`dcmdjpeg` 等未使用工具；源码部署或自定义路径请选择同时包含这四个程序的 DCMTK 3.7.0 `bin` 目录。
...
自动测试覆盖登录/会话/CSRF、Web API、离线静态资源、浏览器断开后的后台任务生命周期和任务恢复。真实 DICOM 集成测试覆盖每 Profile 独立 `storescp`、顺序 `movescu`、多进程端口隔离、压缩传输语法原样接收和跨进程安全归档，并检查输出文件的 `DICM` 标识。
