...
- `DcmGet-2.8.1-Setup-x64.exe`：默认推荐的一键安装器，内置 Python 运行时、PyQt5、pynetdicom、DCMTK 3.7.0、离线中文 OHIF、PDI 本地只读 HTTP 启动器和 Microsoft Visual C++ x64 Runtime，并创建仅允许实际 `DcmGet.exe` 在域/专用网络建立入站连接的程序级防火墙规则，兼容设置中的自定义接收端口。
- `DcmGet-2.8.1-windows-x64-portable.exe`：无需安装的单文件便携版；首次启动需要等待程序解压运行环境，PDI 同样使用原始 DICOM 和离线 OHIF。
- `DcmGet-2.8.1-windows-x64.zip`：解压后直接运行的独立目录版，包含与安装版一致的离线 OHIF 和 PDI 启动器。
...
python scripts/build_windows.py --version 2.8.1
```
...
7. 界面日志默认只显示错误，便于快速定位需要处理的问题；勾选“显示详细日志”后才显示调试、信息、成功和警告。日志仍可在“本任务/全部”之间切换；“清空显示”只清空界面缓存，磁盘中的任务日志与共享接收器日志始终完整保留。
