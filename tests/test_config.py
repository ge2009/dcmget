...
def test_version_six_configuration_adds_delivery_safety_defaults_without_overwriting():
    ...
    configured = AppConfig.from_dict(
        {
            "config_version": 7,
            "minimum_free_space_bytes": str(8 * 1024**3),
            "auto_retry_attempts": "4",
            "circuit_breaker_failures": "7",
            "pdi_volume_size_bytes": str(32 * 1024**3),
        }
    )
    ...
    assert migrated.pdi_volume_size_bytes == 0
    assert migrated.pacs_server_ip == "10.1.2.3"
    assert configured.minimum_free_space_bytes == 8 * 1024**3
    assert configured.auto_retry_attempts == 4
    assert configured.circuit_breaker_failures == 7
    assert configured.pdi_volume_size_bytes == 32 * 1024**3
